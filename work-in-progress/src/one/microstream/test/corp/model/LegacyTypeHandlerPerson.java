...
//		this.initialize(1000040);
	...
	public void update(final Binary bytes, final Person instance, final PersistenceLoadHandler handler)
	{
		...
		final String  contactId =  (String)handler.lookupObject(bytes.get_long(BINARY_OFFSET_contactId));
		final Address address   = (Address)handler.lookupObject(bytes.get_long(BINARY_OFFSET_address  ));
		final String  note      =  (String)handler.lookupObject(bytes.get_long(BINARY_OFFSET_note     ));
		final String  firstname =  (String)handler.lookupObject(bytes.get_long(BINARY_OFFSET_firstname));
		final String  lastname  =  (String)handler.lookupObject(bytes.get_long(BINARY_OFFSET_lastname ));
