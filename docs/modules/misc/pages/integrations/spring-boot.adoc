...
It is also possible to get the entire configuration in the `StorageManagerConfiguration` Bean, which allows you to directly create a foundation and storage manager. This can be useful if you need to stop storage at runtime and restart storage.
